...
impl Visitor<'_> {
    ...
    fn emit_str(&mut self, span: proc_macro2::Span, path_str: String) {
        ...
        self.out.emit(module_path_finding(
            self.file_path,
            span,
            self.severity,
            path_str,
...
fn module_path_finding(
    ...
    let primary = Span::from_pm_span(file_path, span);
    Finding::from_rule(
        AbsoluteModulePathsRule::static_info(),
        severity,
        format!("qualified module path: {path_str}"),
    )
    .with_primary(primary.clone())
    .with_help(String::from("Import the item and use the local name."))
    .with_tags(Vec::from([String::from("imports"), String::from("style")]))
    .with_labels(Vec::from([FindingLabel {
        kind: FindingLabelKind::Primary,
        span: primary,
        message: Some(String::from("qualified path used here")),
    }]))
    .with_notes(Vec::from([FindingNote {
        kind: FindingNoteKind::Help,
        message: String::from("Import the item and use the local name."),
    }]))
    .with_fixes(fixes)
}
