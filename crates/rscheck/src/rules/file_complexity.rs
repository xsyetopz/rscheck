        ...
        for file in &ws.files {
            ...
            match cfg.mode {
                ComplexityMode::Cyclomatic => {
                    ...
                    out.record_metrics(file_metrics(&file.path, sum, max_fn));

                    let over_file = sum > cfg.max_file;
                    let over_fn = max_fn > cfg.max_fn;
                    if over_file || over_fn {
                        out.emit(cyclomatic_finding(
                            &cfg, &file.path, sum, max_fn, over_file, over_fn, &v.per_fn,
                        ));
                    }
                }
                ComplexityMode::PhysicalLoc => {
                    ...
                    if loc as u32 > cfg.max_file {
                        out.emit(physical_loc_finding(&cfg, &file.path, loc));
                    }
                }
                ComplexityMode::LogicalLoc => {
                    ...
                    let ll = v.stmts;
                    if ll > cfg.max_file {
                        out.emit(logical_loc_finding(&cfg, &file.path, ll));
                    }
                }
            }
        }
...
fn file_metrics(path: &Path, cyclomatic_sum: u32, cyclomatic_max_fn: u32) -> FileMetrics {
    FileMetrics {
...
fn cyclomatic_finding(
    ...
    Finding::from_rule(
        ...
        String::from(message.trim_end()),
    )
    .with_primary(file_span(path))
    .with_help(String::from(
        "Split the functions or module to reduce branching.",
    ))
    .with_evidence(format_per_fn(per_fn))
    .with_tags(Vec::from([String::from("complexity")]))
}

fn physical_loc_finding(cfg: &FileComplexityConfig, path: &Path, loc: usize) -> Finding {
    Finding::from_rule(
        FileComplexityRule::static_info(),
        cfg.level.to_severity(),
        format!("file physical LOC {loc} exceeds {}", cfg.max_file),
    )
    .with_primary(file_span(path))
    .with_help(String::from("Split the file into smaller modules."))
    .with_tags(Vec::from([String::from("size")]))
}

fn logical_loc_finding(cfg: &FileComplexityConfig, path: &Path, logical_loc: u32) -> Finding {
    Finding::from_rule(
        FileComplexityRule::static_info(),
        cfg.level.to_severity(),
        format!("file logical LOC {logical_loc} exceeds {}", cfg.max_file),
    )
    .with_primary(file_span(path))
    .with_help(String::from("Split the file into smaller modules."))
    .with_tags(Vec::from([String::from("size")]))
...
fn format_per_fn(per_fn: &[FnScore]) -> String {
    ...
    for s in per_fn {
        let _ = writeln!(&mut out, "{}: {}", s.name, s.score);
    }
    out
}
