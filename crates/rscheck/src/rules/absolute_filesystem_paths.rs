...
impl Visitor<'_> {
    ...
    fn check_str(&mut self, span: proc_macro2::Span, value: &str) {
        ...
        let primary = Span::from_pm_span(self.file_path, span);
        self.out.emit(
            Finding::from_rule(
                AbsoluteFilesystemPathsRule::static_info(),
                self.severity,
                format!("absolute filesystem path ({kind}): {value}"),
            )
            .with_primary(primary.clone())
            .with_help(String::from(
                "Prefer relative paths or build paths via `PathBuf` at runtime.",
            ))
            .with_tags(Vec::from([String::from("paths")]))
            .with_labels(Vec::from([FindingLabel {
                kind: FindingLabelKind::Primary,
                span: primary,
                message: Some(format!("{kind} path literal")),
            }]))
            .with_notes(Vec::from([FindingNote {
...
fn scan_line_comments(
    ...
    for (idx, line) in text.lines().enumerate() {
        ...
        for part in trimmed.split_whitespace() {
            ...
            emit_comment_path_finding(out, file_path, idx, severity, candidate);
        }
    }
}

fn emit_comment_path_finding(
    out: &mut dyn Emitter,
    file_path: &Path,
    idx: usize,
    severity: Severity,
    ...
    let end_location = Clone::clone(&location);
    out.emit(
        Finding::from_rule(
            AbsoluteFilesystemPathsRule::static_info(),
            severity,
            format!("absolute filesystem path in comment: {candidate}"),
        )
        .with_primary(Span::new(file_path, location, end_location))
        .with_tags(Vec::from([String::from("paths"), String::from("comments")])),
    );
}
