...
impl DependencyVisitor<'_> {
    fn check_path(&mut self, span: proc_macro2::Span, path: &syn::Path) {
            ...
            .find(|prefix| matches_path_prefix(&text, prefix))
        {
            self.out.emit(
                Finding::from_rule(
                    BannedDependenciesRule::static_info(),
                    self.severity,
                    format!("banned dependency path: {text}"),
                )
                .with_primary(Span::from_pm_span(self.file, span))
                .with_help(format!("Remove or replace dependency on `{prefix}`."))
                .with_tags(Vec::from([String::from("dependencies")])),
            );
        }
    }
}
